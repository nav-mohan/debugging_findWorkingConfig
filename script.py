...
def find_equilibrium_fcc(model,species, ncells_per_side = NCELLS_PER_SIDE, grid_stepsize=0.01, min_alat = 2.5, max_alat = 10.0):

    # Check if this species has non-trivial force and energy interactions
    print("check atoms_interacting")
    atoms_interacting_energy, atoms_interacting_force = kim_ase_utils.check_if_atoms_interacting(
        model, symbols=[species, species]
    )
    print("done check atoms_interacting")
    if not atoms_interacting_energy:
        ...
        print(
            ...
            "Check. Skipping...".format(model, species)
        )
        print("")
        return -1
...
def main():
    for data in input_data:
        ...
        for spec in species:
            ...
            if model[:5] != "Torch":
                print("FINDING EQUIL")
                spec_equil_alat = find_equilibrium_fcc(model,spec) # dont evaluate nelder-mead for ML models
                print("FOUND EQUIL", spec_equil_alat)
            ...
            spec_work_config['spec_equil_alat'] = spec_equil_alat
            ...
            results.append(spec_work_config)
